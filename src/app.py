...
st.set_page_config(page_title="Chat with MySQL Databases", page_icon=":speech_balloon:")
st.title("Chat with MySQL Databases")

# Function to initialize the database connection
def init_database(user: str, password: str, host: str, port: str, database: str) -> SQLDatabase:
    db_uri = f"mysql+mysqlconnector://{user}:{password}@{host}:{port}/{database}"
    print(f"DB URI: {db_uri}")  # Debug print
...
with st.sidebar:
    ...
    if st.button("Connect"):
        with st.spinner("Connecting to database..."):
            try:
                print(f"Connecting with: {user}, {password}, {host}, {port}, {database}")  # Debug print
